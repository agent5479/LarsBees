...
// Migration function for user-based to tenant-based structure
window.migrateUserDataToTenant = function() {
    console.log('🔄 Migrating user data to tenant structure...');
    const currentUserId = localStorage.getItem('currentUserId');
    const currentTenantId = localStorage.getItem('currentTenantId');
    
    if (!currentUserId || !currentTenantId) {
        console.error('❌ Missing user ID or tenant ID');
        alert('❌ Missing user ID or tenant ID. Please log in again.');
        return;
    }
    
    console.log('📊 Migrating data for user:', currentUserId, 'to tenant:', currentTenantId);
    ...
    let migrationPromises = [];
    let migrationCount = 0;
    const totalDataTypes = 4; // clusters, actions, tasks, individualHives
    
    function checkMigrationComplete() {
        migrationCount++;
        console.log(`📊 Migration progress: ${migrationCount}/${totalDataTypes}`);
        
        if (migrationCount >= totalDataTypes) {
            console.log('🎉 User data migration completed successfully!');
            showSyncStatus('<i class="bi bi-check-circle"></i> Migration Complete', 'success');
    ...
    // Migrate clusters to sites
    database.ref(`tenants/${currentTenantId}/clusters`).once('value', (snapshot) => {
        const oldClusters = snapshot.val();
        if (oldClusters) {
            console.log('📦 Migrating clusters to sites...');
            migrationPromises.push(
                database.ref(`tenants/${currentTenantId}/sites`).set(oldClusters).then(() => {
                    console.log('✅ Clusters migrated to sites successfully');
            ...
            console.log('📭 No clusters found to migrate');
            checkMigrationComplete();
        }
    });
    
    // Actions are already in the correct location, just check if they exist
    database.ref(`tenants/${currentTenantId}/actions`).once('value', (snapshot) => {
        const actions = snapshot.val();
        if (actions) {
            console.log('✅ Actions already in correct location');
        } else {
    ...
    // Tasks are already in the correct location, just check if they exist
    database.ref(`tenants/${currentTenantId}/tasks`).once('value', (snapshot) => {
        const tasks = snapshot.val();
        if (tasks) {
            console.log('✅ Tasks already in correct location');
        } else {
    ...
    // Individual hives are already in the correct location, just check if they exist
    database.ref(`tenants/${currentTenantId}/individualHives`).once('value', (snapshot) => {
        const hives = snapshot.val();
        if (hives) {
            console.log('✅ Individual hives already in correct location');
        } else {
            console.log('📭 No individual hives found');
        }
        checkMigrationComplete();
    });
};
