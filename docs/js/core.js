...
window.migrateUserDataToTenant = function() {
    ...
    database.ref(`users/${currentUserId}/clusters`).once('value', (snapshot) => {
        const oldClusters = snapshot.val();
        if (oldClusters) {
    ...
    // Migrate actions
    database.ref(`users/${currentUserId}/actions`).once('value', (snapshot) => {
        const oldActions = snapshot.val();
        if (oldActions) {
            console.log('📦 Migrating actions...');
            migrationPromises.push(
                database.ref(`tenants/${currentTenantId}/actions`).set(oldActions).then(() => {
                    console.log('✅ Actions migrated successfully');
                    checkMigrationComplete();
                }).catch(error => {
            ...
            console.log('📭 No actions found to migrate');
            checkMigrationComplete();
        }
    });
    
    // Migrate tasks
    database.ref(`users/${currentUserId}/tasks`).once('value', (snapshot) => {
        const oldTasks = snapshot.val();
        if (oldTasks) {
            console.log('📦 Migrating tasks...');
            migrationPromises.push(
                database.ref(`tenants/${currentTenantId}/tasks`).set(oldTasks).then(() => {
    ...
    // Migrate individual hives
    database.ref(`users/${currentUserId}/individualHives`).once('value', (snapshot) => {
        const oldHives = snapshot.val();
        if (oldHives) {
            console.log('📦 Migrating individual hives...');
            migrationPromises.push(
                database.ref(`tenants/${currentTenantId}/individualHives`).set(oldHives).then(() => {
                    console.log('✅ Individual hives migrated successfully');
                    checkMigrationComplete();
                }).catch(error => {
                    console.error('❌ Error migrating individual hives:', error);
                    checkMigrationComplete();
                })
            );
        } else {
            console.log('📭 No individual hives found to migrate');
            checkMigrationComplete();
        }
    });
};
